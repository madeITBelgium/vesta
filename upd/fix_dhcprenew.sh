...
if [ -e "/usr/bin/dhcprenew" ]; then
    ...
    /usr/local/vesta/bin/v-add-user-notification admin \
        "Security Check" "Your server was compromised please contact us at info@madeit.be to get help."
    ...
    if [ ! -z "$email" ]; then
        echo "Your server $(hostname) was compromised please contact us at info@madeit.be to get help." |\
